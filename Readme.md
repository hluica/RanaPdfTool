...
> **辅助 Powershell 脚本**
>
> 本存储库附带了一个 [Powershell 脚本](Merge-Pdf.ps1)，此脚本提供了对多个文件夹进行批量合并文件的功能。
>
> 用法：`.\Merge-Pdf.ps1 -Source <Source Folder> -Target <Target Folder>` 脚本会在源文件夹下搜索子文件夹（不递归；内部子文件夹会被转换为 PDF 目录结构），然后将每个子文件夹的路径传递给 RanaPdfTool.exe，并将输出路径设为目标文件夹。目标文件夹不存在时会自动创建。
>
> 此脚本运行时会显示当前处理文件夹的序号与运行 RanaPdfTool.exe 时的所有输出。如果没有发生错误，则会正常地在运行完成后退出；如果运行中发生了错误，脚本会在所有子文件夹全部处理完成之后显示总结，包含错误文件夹在处理过程中的序号及其名称。前者可用于检查处理过程中的程序输出，后者可用于检查文件。
>
> 说明：此脚本需要配合 PowerShell 7 或更高版本使用，因为其中使用了 PowerShell 7 中引入的 `` `e `` 转义字符。要在 Powershell 5 中使用此脚本，需要将其中的 `` `e `` 转义字符替换为 `$([char]27)`。
...
| 版本号 | 发布日期 | 更新内容                                                                                                                       |
| ------ | -------- | ------------------------------------------------------------------------------------------------------------------------------ |
| v4.2.6 | 26-04-12 | 更新依赖并更新主CommandExecute方法；更新readme。                                                                               |
| v4.2.5 | 26-04-05 | 优化 AnsiConsole 扩展类；改进辅助脚本。                                                                                        |
