...
) : AsyncCommand<MergeSettings>
{
    ...
    protected override async Task<int> ExecuteAsync(CommandContext context, MergeSettings settings, CancellationToken cancellationToken)
    {
        ...
        {
            AnsiConsole.Error.MarkupLine($"[red][[ERROR]][/] Source directory not found: [red underline]{Markup.Escape(sourceDir)}[/]");
            return 1;
        }
        ...
        if (isExplicitFile)
        {
            ...
            {
                AnsiConsole.Error.MarkupLine($"[red][[ERROR]][/] Cannot create file [red underline]{Markup.Escape(Path.GetFileName(rawDestPath))}[/]. A folder with the same name already exists at destination.");
                return 1;
            }
        ...
        else
        {
            ...
            {
                AnsiConsole.Error.MarkupLine($"[red][[ERROR]][/] Destination path [red underline]{Markup.Escape(rawDestPath)}[/] exists and is a file. Please specify a directory or a new .pdf filename.");
                return 1;
            }
        ...
        AnsiConsole.MarkupLine("[gray][[INFO]][/] Scanning directory structure...");
        ...
        else
        {
            ...
            AnsiConsole.Error.MarkupLine($"[red][[ERROR]][/] Completed with [red bold]{errors.Count}[/] errors.");
            if (hasCriticalFailure)
                AnsiConsole.Error.MarkupLine("[red][[ERROR]][/] Including [bold]CRITICAL ERROR[/].");

            AnsiConsole.Error.Write(new Rule("[red]Merge Failures[/]").LeftJustified());
            foreach (var (ctxName, exception) in errors)
            {
                AnsiConsole.Error.MarkupLine($"[gray bold]Item:[/] [underline]{Markup.Escape(ctxName)}[/]");
